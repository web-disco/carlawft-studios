...
const service = services.filter(service => service.contentID === dynamicPageItem.contentID)
  ...
  return (
    ...
    <div>
      ...
      <div className="sm:hidden my-10 grid grid-cols-2 gap-4">
        ...
        <div>
        <Link to="/equipment" className="block border-2 text-center border-black text-black text-sm p-2 hover:bg-black hover:text-white transition ease-in-out duration-300">
          View Equipment
        </Link>
        </div>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 my-6">
            ...
            {rates.map((rate, index) => (
              <div className="flex justify-between mb-4" key={index}>
                <div className="pr-14">
                  ...
                  <div className="prose text-sm max-w-full text-black" dangerouslySetInnerHTML={renderHTML(rate.customFields.service)} />
                </div>
        ...
        <div className={`hidden sm:block col-span-1 ${rates.length > 0 ? `text-right` : `text-left`}`}>
          {dynamicPageItem.customFields.form && (
            <button className="bg-black text-white border-2 border-black text-sm mb-4 w-40 p-2 hover:bg-white hover:text-black transition ease-in-out duration-300" onClick={handleShow}>
              Book Now
            </button>
          )}
          <div>
          <Link to="/equipment" className="border-2 text-center border-black text-black text-sm inline-block w-40 p-2 hover:bg-black hover:text-white transition ease-in-out duration-300">
            View Equipment
          </Link>
          </div>
        </div>
    ...
    >
      {dynamicPageItem.customFields.form && (
        <ServiceForm form={dynamicPageItem.customFields.form} handleClose={handleClose} />
      )}
    </Modal>
    </>
  )
